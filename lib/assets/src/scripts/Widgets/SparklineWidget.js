...
class SparklineWidget {
  ...
  hover(i) {
    ...
    if (this.data[0][i]) {
      ...
      this.metricDataValue.text(formatData(this.data[0][i].y, '', '', false, defined(this.prefix)));
  ...
  getSummary(i) {
    ...
    if (i < this.data[0].length && i > 0) {
      ...
      volume = formatData(Math.abs(this.data[0][i].y - this.data[0][ i - 1].y), this.prefix, this.suffix);
  ...
  getTrend(i){
   ...
   switchColorMode(isHighContrastMode){
    defined(this.sparkline) && this.sparkline.switchColorMode(isHighContrastMode);
  }
