...
d3.selectAll('.sparkline .widget__inner').each(function() {
  ...
  let sparkline = new SparklineWidget({
    ...
    suffix: widget.suffix,
    units: widget.units,
    displayRoundedData: false, //widget.displayRoundedData,
    isHighContrastMode: isHighContrastMode
  });
...
let isDashboardPage = $('body').hasClass('dashboards');
...
function toggleHighContrastMode(value) {
  ...
  charts.forEach((c) => {
    c.switchColorMode(isDashboardPage ? isHighContrastMode : false);
  });
  if (localStorage) {
